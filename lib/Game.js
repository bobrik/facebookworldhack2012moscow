    ...
    Game.prototype.exportFor = function(user, callback) {
        ...
        if (this.data.opponent == userId || this.data.started == userId) {
            ...
            function resume() {
                ...
                result.starterGuesses = self.data.starterGuesses;
                result.opponentGuesses = self.data.opponentGuesses;

                if (userId == self.data.opponent) {
                    result.guessed = self.data.opponentGuesses;
                } else {
                    result.guessed = self.data.starterGuesses;
                }

                if (userId == self.data.opponent) {
                    result.i_hang_up = this.data.firstHangUp;
                    result.i_won     = this.data.secondWon;
                    result.i_hanged  = this.data.secondHangUp;
                    result.i_lost    = this.data.firstWon;

                    result.partner = self.data.started;
                    result.word    = self.data.firstWord;
                } else {
                    result.i_hang_up = this.data.secondHangUp;
                    result.i_won     = this.data.firstWon;
                    result.i_hanged  = this.data.firstHangUp;
                    result.i_lost    = this.data.secondWon;

                    result.partner = self.data.opponent;
                    result.word    = self.data.secondWord;
                }


                result.firstMistakes = self.data.firstMistakes || 0;
                result.secondMistakes = self.data.secondMistakes || 0;

                console.log(self.data.opponentGuesses.length, self.data.starterGuesses.length)

                if (self.data.opponentGuesses.length == self.data.starterGuesses.length && userId == self.data.opponent) {
                    result.move = true;
                } else {
                    result.move = false;
                }
